
Website Reference Documents
---------------------------

:Project: `Website Reference Documents <https://example.com/index.html>`_
:Author: `Jay Gray <https://ontomatica.io>`_
:Organization: `Ontomatica <https://dailyfood.io>`_
:Date: 27 May 2023

TEXT


.. toctree::
   :maxdepth: 2
   :caption: 🍱 Contents

   acceptance-test
   amp-dev-google-dialog
   amp-dev-installation-notes
   amp-dev-search
   amp-dev-server-side-processes
   amp-dev-structure
   amp-dev-toolchain
   amp-dev-visualization
   content-substitution-plan
   div-dev-css
   div-dev-how-it-works
   div-dev-internationalization
   frequently-asked-questions
   grow-static-site-generator
   gulp-amp-dev
   organization-background
   workflow-ci-cd
   23-04-22
   22-07-13
   22-06-20
   22-02-28
   22-02-22


